  ...
  const fetchData = async () => {
    ...
    try {
      const response = await postMethod(
        `career-list?search=${searchTerm || ''}`,row
      );
      console.log(response.data.data[0], 'ff1');
      ...
      console.error('Error fetching data0:', error);
      setLoading(false);
    }
  };
  ...
  return (
    <View style={[styles.container, {backgroundColor: color}]}>
      <View style={styles.row}>
        <Pressable style={{flexDirection: 'row', gap: 10, alignItems: 'center'}} onPress={() => handleBackPress()}>
          {icon !== null ? (
