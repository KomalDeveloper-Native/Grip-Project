...
const Profile: FC<Props> = ({navigation}: any): JSX.Element => {
  ...
  const onRefresh = async () => {
    ...
    try {
      await setValueFun(); // Re-fetch profile data
    } catch (error) {
  ...
  const setValueFun = async () => {
    try {
      ...
      console.log(response.data, id, 'data0');
      ...
      if (response.data && response.data.data) {
        ...
        setValue('email_id', data.email);
        setValue('gender', data.gender);
        setValue('phone_no', data.phone_number);
        ...
        setSelectedDate(data.date_of_birth); // Fixed typo from 'birtday' to 'birthday'
      }
    } catch (error) {
      ...
      <View style={styles.container}>
        ...
        <View
          ...
          <Text style={{fontFamily: 'Roboto-Bold'}}>
            <Text style={{fontFamily: 'Roboto-Bold' ,fontSize:responsiveFontSize(2)}}>Profile details</Text>
          </Text>
        </View>
