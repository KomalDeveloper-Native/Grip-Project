...
interface Props {}
export const FranchiseApply: FC<Props> = ({navigation, route}): JSX.Element => {
  ...
  useMemo(async () => {
    ...
    console.log(storage.response.user, 'ste', franchiseid);
  ...
  const fetchFrachise = async data => {
    ...
    try {
      ...
      const response: any = await postMethod('franchise-apply', row);
      ...
      if (response.status === 200) {
        if (franchiseid) {
          navigation.dispatch(
            CommonActions.reset({
              index: 1,
              routes: [{ name: 'FranchiseScreen' }], // Replace with your desired route
            })
          );
         } else {
    ...
    <>
      ...
      <View style={styles.container}>
        <Text style={styles.title}>
          Apply Franchise for Kayakalp the academy of yoga
        </Text>
