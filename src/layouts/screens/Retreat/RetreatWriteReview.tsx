...
import {FormPostMethod, getMethod, getStorageData} from '../../../utils/helper';
...
interface Props {}
const RetreatWriteReview: FC<Props> = ({navigation, route}): JSX.Element => {
  ...
  const {retreatid, retreat} = route.params;
  console.log(retreat, retreatid, 'coi');
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      ...
      formData.append('retreat_id', retreatid);
      ...
      if (response.data.success === true) {
        ...
        if (retreatid && retreat) {
          navigation.navigate('RetreatDetailsScreen', {
            retreatid: retreatid,
            retreat: retreat,
          });
        } else {
          console.log('retreatid or retreat is undefined');
        }
      } else {
        Snackbar.show({
      ...
      <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
        <KeyboardAvoidingView enabled>
          <View style={styles.containerRow}>
            <Image source={{uri: retreat.image}} style={styles.image} />
            <View>
              <Text style={styles.text1}>{retreat.title}</Text>
            </View>
          </View>
          <View
            style={[
...
export default RetreatWriteReview;
