...
interface Props {}
const EditSeco: FC<Props> = ({navigation, course}: any): JSX.Element => {
  ...
  useFocusEffect(
    useCallback(()=>{
      if (course) {
        setValue('meta_title', course.meta_title);
        setValue('meta_keywords', course.meta_keywords);
  ...
  const updateProfile = async (data: {
    ...
    try {
      ...
      const response: any = await postMethod(
        `update-course-seo?id=${course.id}`,
        row,
      );
