...
const NewCourseScreen: FC<Props> = ({navigation}: any): JSX.Element => {
  ...
  const data1 = [
    {name: 'In-Person Class', id: 'In-Person Class'},
    {name: 'Virtual Class', id: 'Virtual Class'},
  ];
  ...
  const createCourse = async (data: IFormData) => {
    ...
    console.warn(storage.response, 'value');
    ...
    try {
      ...
      const response: any = await FormPostMethod('addcourse', formData);
      console.log(response.data);
      if (response.data.success === true) {
        setClick(true);
        setNewCourse(response.data.id);
        console.log(selectedWeek, response.data, 'ifmm');
        Snackbar.show({
  ...
  return (
    <>
      <View style={styles.row0}>
        ...
        />
        <Text style={styles.text0}>Create New Course</Text>
      </View>
      <ScrollView keyboardShouldPersistTaps="handled">
        <KeyboardAvoidingView enabled>
          <View style={styles.container}>
            <Text style={styles.title}>Enter Course Title </Text>
            <Controller
              control={control}
              rules={{
                required: true,
