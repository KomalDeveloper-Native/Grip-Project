...
interface Props {}
const userRetreat: FC<Props> = ({navigation, route}): JSX.Element => {
  ...
  const userRetreatLisfun = async () => {
    const storage = await getStorageData();
    const id = storage.response.user.id;
    let endpoint = ``;
    setLoading(true);
    try {
      endpoint = `simple-user-retreat-lead-list?user_id=${id}`;
      console.log(endpoint, 'endpo');
      const response: any = await getMethod(endpoint);
      if (response.status === 200) {
        setRetreat(response.data.lead_list);
      ...
      <View style={styles.container}>
        <View
          ...
          <Text style={{fontFamily: 'Roboto-Bold'}}>
            <Text style={{fontFamily: 'Roboto-Bold' ,fontSize:responsiveFontSize(2)}}>My retreat bookings</Text>
          </Text>
        </View>
        <ScrollView
          ...
          {loading ? (
            ...
            retreat.map(item => (
              <Pressable
                style={styles.row}
                key={item.id}
                onPress={() => ListDetailFun(item.lead_id)}>
                <View style={styles.row1}>
                  ...
                  />
                  <View>
                    <Text style={styles.name}>{item.retreat_title}</Text>
